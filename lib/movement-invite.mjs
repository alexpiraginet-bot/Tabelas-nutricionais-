...
import { supabaseServiceHeaders } from "./supabase-rest.mjs";
...
export async function claimMovementInviteResponse({ fetchImpl, cfg, inviteId, now }) {
  ...
  const response = await fetchImpl(`${cfg.url}/rest/v1/movement_invites?id=eq.${encodeURIComponent(inviteId)}&status=in.(sent,opened,responded)&revoked_at=is.null`, {
    ...
    body: JSON.stringify({ status: "responded", updated_at: timestamp }),
  });
...
export async function resolveMovementInvite({ fetchImpl, cfg, token, now, markOpened = false }) {
  ...
  if (!row) {
    const aliasResponse = await fetchImpl(`${cfg.url}/rest/v1/movement_invite_aliases?token_hash=eq.${tokenHash}&select=invite_id&limit=1`, { headers: supabaseServiceHeaders(cfg.key) });
    if (!aliasResponse.ok) throw await supabaseFailure(aliasResponse);
    const aliases = await aliasResponse.json();
    const inviteId = Array.isArray(aliases) ? aliases[0]?.invite_id : null;
    if (inviteId) {
      const inviteResponse = await fetchImpl(`${cfg.url}/rest/v1/movement_invites?id=eq.${encodeURIComponent(inviteId)}&select=${select}&limit=1`, { headers: supabaseServiceHeaders(cfg.key) });
      if (!inviteResponse.ok) throw await supabaseFailure(inviteResponse);
      const invites = await inviteResponse.json();
      row = Array.isArray(invites) ? invites[0] : null;
    }
  }
  ...
  if (markOpened && row.opened_at == null) {
    ...
    const openedRows = await opened.json();
    if (!Array.isArray(openedRows) || openedRows.length === 0) return null;
  }
