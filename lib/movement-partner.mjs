...
export const PARTNER_TIER_OPTIONS = ["select", "experience", "signature", "founding_circle"];
export const LEGACY_PARTNER_TIER_OPTIONS = ["founding", "experience", "kit", "mobility", "support", "custom"];
...
export function validatePartnerLead(body = {}, { allowedTiers = PARTNER_TIER_OPTIONS } = {}) {
  const errors = {};
  const companyName = clean(body.companyName, 120);
  const contactName = clean(body.contactName, 120);
  const email = clean(body.email, 160).toLowerCase();
  const phone = clean(body.phone, 32);
  const tier = allowedTiers.includes(body.tier) ? body.tier : null;
